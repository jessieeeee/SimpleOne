                                                                                                                                                                                             ...
                                                                                                                                                                                             */function find(roots, extensions, ignore, callback)
{
  ...
  function search(directory) {
    activeCalls++;
    _fs2.default.readdir(directory, (err, names) => {
      activeCalls--;
      ...
      names.forEach(file => {
        ...
        _fs2.default.lstat(file, (err, stat) => {
          activeCalls--;
          ...
          if (!err && stat && !stat.isSymbolicLink()) {
            if (stat.isDirectory()) {
              ...
              const ext = _path2.default.extname(file).substr(1);
...
callback)
{
  ...
  child.stdout.on('close', () => {
    ...
    if (!count) {
      ...
      lines.forEach(path => {
        _fs2.default.stat(path, (err, stat) => {
          if (!err && stat) {
  ...
  return new Promise(resolve => {
    const callback = list => {
      ...
      list.forEach(fileData => {
        ...
        if (existingFile && existingFile[_constants2.default.MTIME] === mtime) {
